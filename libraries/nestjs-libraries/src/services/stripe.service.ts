...
export class StripeService {
  ...
  async refundCharges(organizationId: string, chargeIds: string[]) {
    ...
    if (!org?.paymentId) {
      throw new Error(
        'Nenhum cliente de pagamento foi encontrado para esta organização'
      );
    }
  ...
  async cancelSubscription(organizationId: string) {
    ...
    if (!subscriptions.length) {
      throw new Error('Nenhuma assinatura ativa foi encontrada');
    }
  ...
  async lifetimeDeal(organizationId: string, code: string) {
    ...
    if (getCurrentSubscription && !getCurrentSubscription?.isLifetime) {
      throw new Error('Você já possui uma assinatura que não é vitalícia');
    }
