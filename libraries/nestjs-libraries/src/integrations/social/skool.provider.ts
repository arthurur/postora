...
export class SkoolProvider extends SocialAbstract implements SocialProvider {
  ...
  override handleErrors(
    ...
    if (body.includes('must be admin or level')) {
      return { type: 'bad-body', value: 'Você não pode publicar neste canal' };
    }
    if (body.includes('cannot post to this label')) {
      return {
        type: 'bad-body',
        value: 'Não é possível publicar neste rótulo',
      };
    }
    return undefined;
  }
  ...
  async authenticate(params: {
    ...
    try {
      ...
      if (missing.length > 0) {
        return `Os cookies obrigatórios estão ausentes: ${missing.join(', ')}`;
      }
      ...
      return 'Dados de cookies inválidos';
    }
  }
