...
const getAspectRatio = (t: (key: string, fallback: string) => string) => [
  { key: 'portrait', value: t('heygen_portrait', 'Portrait') },
  { key: 'story', value: t('heygen_story', 'Story') },
];

const getGenerateCaptions = (t: (key: string, fallback: string) => string) => [
...
const HeygenProviderComponent = () => {
  const t = useT();
  const aspectRatio = getAspectRatio(t);
  const generateCaptions = getGenerateCaptions(t);
    ...
    resolver: zodResolver(
      object({
        ...
        selectedVoice: string().min(
          1,
          t('heygen_voice_required', 'Voice is required')
        ),
        aspect_ratio: string().min(
          1,
          t('heygen_aspect_ratio_required', 'Aspect ratio is required')
        ),
        captions: string().min(
          1,
          t('heygen_captions_required', 'Captions is required')
        ),
      })
    ),
  });
  ...
  const generateVoice = useCallback(async () => {
    if (
      !(await deleteDialog(
        t(
          'heygen_confirm_delete_current_text',
